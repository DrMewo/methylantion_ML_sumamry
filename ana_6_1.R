for(k in c(1:30)){
     para_name<-paste("/mnt/ilustre/users/sanger-dev/sg-users/zhangpeng/fighting/data_para/1/data",k,sep="_")
     ...
     for(j in c(1:all_len)){
        ...
        for(i in c(1:length(a))){
            ...
            k<-c[which(c[,1]==names(table(c[,1]))[i]),2]
            k2<-c[-which(c[,1]==names(table(c[,1]))[i]),2]
            zp_table[i+1,2]<-wilcox.test(k,k2,exact=FALSE,correct=FALSE)$p.value
...
para_name_summary_10<-paste("/mnt/ilustre/users/sanger-dev/sg-users/zhangpeng/fighting/data_para/1/data_summary_10",k,sep="_")
