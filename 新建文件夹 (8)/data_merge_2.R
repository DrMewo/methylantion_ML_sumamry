file_path<-"/mnt/ilustre/users/sanger-dev/sg-users/zhangpeng/cancer_data_part2/gdc"
    ...
    if(list.files(firstfile)[1]=="logs")
        {
            ...
            if(list.files(firstfile)[2]=="annotations.txt"){
                secondfile<-list.files(firstfile)[3]
            }else{
                secondfile<-list.files(firstfile)[2] 
            }
        }else{
            #secondfile<-list.files(firstfile)[1]
            if(list.files(firstfile)[1]=="annotations.txt"){
                secondfile<-list.files(firstfile)[2]
            }else{
                secondfile<-list.files(firstfile)[1]
            }
    ...
    if(length(data_para[,1])==485577)
        {
            ...
            data_para_1<-merge(data_para_1,map_list,x.by="names",y.by="names")
               data_para_2<-data_para_1[,c(1,2)]
               colnames(data_para_2)<-c("names",secondfile)
            }else{
               data_para_1<-data_para[,c(1,2)]
               colnames(data_para_1)<-c("names","Beta_value")
...
write.table(new_data,"/mnt/ilustre/users/sanger-dev/sg-users/zhangpeng/fighting/gogo/cal_data_2.txt",row.names = FALSE)
